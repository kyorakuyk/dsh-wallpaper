...
设置中心的“登录过渡底图”会通过 Windows 8+ 的 `IDesktopWallpaper` 接口把 Explorer 普通桌面壁纸切换为内置 `sleep.png`，并立即回读确认；让用户输入密码后到应用原生首帧接管之间不再闪出用户原壁纸。它不是 Shell 替换，也不参与密码页；应用只保存当前静态壁纸的恢复点，遇到 Spotlight、幻灯片、动态壁纸或检测到用户已改过壁纸时会拒绝覆盖。旧版或 COM 不可用时才使用带结果校验的兼容接口。

该开关默认关闭。启用前请确认当前桌面使用的是可读的静态图片；关闭时应用只在当前壁纸仍是它管理的睡眠图时恢复原图。若外部程序已经更换壁纸，恢复点会保留并显示冲突提示，不会强行覆盖。清理过期恢复点会永久删除保存的原图副本，设置中心会在执行前再次确认；卸载前应先在设置中心关闭该开关。
