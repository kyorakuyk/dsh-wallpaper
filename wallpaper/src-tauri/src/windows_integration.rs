...
#[cfg(windows)]
unsafe extern "system" fn find_worker(window: HWND, lparam: LPARAM) -> BOOL {
    let shell_view = FindWindowExW(Some(window), None, windows::core::w!("SHELLDLL_DefView"), PCWSTR::null());
    if shell_view.is_ok() {
        if let Ok(worker) = FindWindowExW(None, Some(window), windows::core::w!("WorkerW"), PCWSTR::null()) {
...
pub fn attach_to_workerw(window: &WebviewWindow) -> Result<(), String> {
    ...
    unsafe {
        ...
        let parent = if worker.0.is_null() { GetDesktopWindow() } else { worker };
        SetParent(hwnd, Some(parent)).map_err(|e| e.to_string())?;
        let style = GetWindowLongPtrW(hwnd, GWL_EXSTYLE);
        SetWindowLongPtrW(hwnd, GWL_EXSTYLE, style | WS_EX_TOOLWINDOW.0 as isize | WS_EX_NOACTIVATE.0 as isize);
        let _ = windows::Win32::UI::WindowsAndMessaging::SetWindowPos(hwnd, Some(HWND_BOTTOM), 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
    }
    Ok(())
}
